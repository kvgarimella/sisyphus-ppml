...
parser = argparse.ArgumentParser(description='PyTorch CIFAR10 Poly Regression Test Function')
...
if args.model == "alexnet_bn":
    ...
    import resnet18_bn
    net = resnet18_bn.ResNet18()
    net = net.to(device)
elif args.model == "mobilenetv1_bn":
    import mobilenetv1 
    net = mobilenetv1.MobileNetV1(num_classes=10)
    net = net.to(device)
elif args.model == "resnet32_bn":
    import resnet32_bn
    net = resnet32_bn.ResNet32(num_classes=10)
    net = net.to(device)
else:
...
os.system("mkdir -p bayes_data_cifar10")
torch.save(train_x, "bayes_data_cifar10/xs_{}.pth".format(args.model))
torch.save(train_obj, "bayes_data_cifar10/ys_{}.pth".format(args.model))
