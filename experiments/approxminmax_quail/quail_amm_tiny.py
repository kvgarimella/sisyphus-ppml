...
parser = argparse.ArgumentParser(description='PyTorch TinyImageNet QuaIL+AMM Training')
...
for i in range(num_layers-1):
    if channels[i][0] == 1:
        student.module.change_activation(i, nn.Sequential(ApproxMinMaxNorm1d(num_features=channels[i][1], mode=args.approxmode).to(device), Quad()))
    else:
        student.module.change_activation(i, nn.Sequential(ApproxMinMaxNorm2d(num_features=channels[i][1], mode=args.approxmode).to(device), Quad()))
...
def train(args, student, teacher, device, trainloader, optimizer, epoch, layer):
    ...
    avg_loss = avg_sum_loss / (batch_idx+1)
...
os.system("mkdir -p approx_quail_nets_tiny")

torch.save(student.state_dict(), "approx_quail_nets_tiny/{}_stage1.pth".format(args.model))
## Fine-tune Stage
student.module.freeze_all()
student.module.print_all()
...
test(args, student, device, testloader, epoch)
torch.save(student.state_dict(), "approx_quail_nets_tiny/{}_stage2.pth".format(args.model))
wandb.save("approx_quail_nets_tiny/{}_{}_stage1.pth".format(args.model, args.approxmode))
wandb.save("approx_quail_nets_tiny/{}_{}_stage2.pth".format(args.model, args.approxmode))
